...
func (s *state) visitSoyFile(node *ast.SoyFileNode) {
	...
	s.jsln("")
	s.jsln("/* START */")
	s.jsln("// declared: [", strings.Join(removeDuplicates(s.funcsInFile), ", "), "]")
	s.jsln("// called: [", strings.Join(removeDuplicates(s.funcsCalled), ", "), "]")
	s.jsln("/* END */")
	s.jsln("")
}
...
func (s *state) visitTemplate(node *ast.TemplateNode) {
	...
	s.jsln("")
	s.jsln("export function ", strings.Replace(node.Name, ".", "__", -1), "(opt_data, opt_sb, opt_ijData) {")
	s.funcsInFile = append(s.funcsInFile, strings.Replace(node.Name, ".", "__", -1))
	s.indentLevels++
...
func (s *state) visitPrint(node *ast.PrintNode) {
	...
	for _, dir := range directives {
		if strings.HasPrefix(PrintDirectives[dir.Name].Name, "Yext") {
			s.js(dir.Name, "(")
			s.funcsCalled = append(s.funcsCalled, "directives."+dir.Name)
		} else {
...
func (s *state) visitFunction(node *ast.FunctionNode) {
	if fn, ok := Funcs[node.Name]; ok {
		...
		s.funcsCalled = append(s.funcsCalled, "funcs."+node.Name)
		return
	}
...
func (s *state) visitCall(node *ast.CallNode) {
	...
	s.jsln(s.bufferName, " += ", strings.Replace(node.Name, ".", "__", -1), "(", dataExpr, ", opt_sb, opt_ijData);")
	s.funcsCalled = append(s.funcsCalled, strings.Replace(node.Name, ".", "__", -1))
}
